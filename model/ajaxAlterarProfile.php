...
<?php 
  ...
  if ($nome != "empty") {
   ...
   if ($unidade != 0 && $unidade != 1) {
     mysql_query("UPDATE tb_leads SET id_unidade = '".$unidade."', esta_ativo = 1, id_status = 2 WHERE id = ".$id);
     ...
     date_default_timezone_set('America/Sao_Paulo');
      $date = date('d-m-Y H:i');
     mysql_query("INSERT tb_email_queue SELECT null,(select distinct email b from tb_usuarios a inner join tb_unidades b on a.id_unidade = b.id where b.id = ".$unidade." AND a.id_level != 5),'".$date."',".$id); 

   } elseif ($unidade == 1) {
     mysql_query("UPDATE tb_leads SET id_unidade = '".$unidade."', esta_ativo = 1, id_status = 1 WHERE id = ".$id);
